...
## May 16 2024. Not needed right now. All spp coded correctly in the xml
...
for (k in seq_len(n_dif)) {
  ...
  if(is.null(keywords)) { nullKeywords[k] <- TRUE}
  ...
  ## May 16 2024 -- no longer happens after Ed G's edits of the bad files.
  ## Except for the Zhao et al zip file
...
# studies without keywords -- send to Ed G for inspection
dif_study[nullKeywords]
...
itrdb_meta[rownames(itrdb_meta) %in% bad_id,]
